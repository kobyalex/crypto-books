...
function geckoCoinsDemo(ui, key, fiat, ids) {
  ...
  enableCache();
  var json = importJson(
    "https://api.coingecko.com/api/v3/coins/markets?vs_currency=" + fiat + "&ids=" + ids + "&x_cg_demo_api_key" + key
  );
  //pro-api.coingecko.com/api/v3/
  https: if (typeof json === "string" && getCoinGeckoDemoKey() == "") {
...
function geckoRateDemo(ui, key, fiat, coin, date) {
  ...
  enableCache();
  var json = importJson(
    "https://api.coingecko.com/api/v3/coins/" + coins[coin] + "/history?date=" + date,
    "market_data.current_price." + fiat + "&x_cg_demo_api_key" + key
  );
...
function geckoFluxDemo(ui, key, fiat, coin, limit, interval) {
  ...
  if (typeof json === "string" && getCoinGeckoDemoKey() == "") {
    ui.alert("CoinGecko API error: " + json);
  } else {
    for (var i in json[0][1]) {
