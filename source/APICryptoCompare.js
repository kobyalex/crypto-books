...
function cryptoCoins(ui, key, fiat, tickers) {
  ...
  disableCache();
  var json = importJson(
    "https://min-api.cryptocompare.com/data/pricemultifull?api_key=" +
      key +
      "&tsyms=" +
      fiat +
      "&fsyms=" +
      tickers +
      "&relaxedValidation=true&extraParams=cryptoBooks2"
  );
...
function cryptoSparkline(ui, key, coin) {
  ...
  enableCache();
  var json = importJson(
    "https://min-api.cryptocompare.com/data/v2/histohour?api_key=" +
      key +
      "&fsym=" +
      coin +
      "&tsym=" +
      fiat +
      "&limit=168&extraParams=cryptoBooks2"
  );
...
function cryptoRate(ui, key, fiat, coin, date) {
  ...
  var json = importJson(
    "https://min-api.cryptocompare.com/data/v2/histohour?api_key=" +
      ...
      "&toTs=" +
      date.getTime() / 1000 +
      "&limit=1&extraParams=cryptoBooks2",
    "Data.Data.0.close"
  );
...
function cryptoFlux(ui, key, fiat, coin, limit, interval) {
  ...
  if (typeof json === "string") {
    ...
    for (var i in json[5][1]["Data"]) {
      flux.push({
        date: new Date(json[5][1]["Data"][i]["time"] * 1000),
        price: json[5][1]["Data"][i]["close"],
        volume: json[5][1]["Data"][i]["volumeto"],
      });
    }
  }
