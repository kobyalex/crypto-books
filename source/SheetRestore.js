...
function sheetRestore(ui) {
  ...
  while (files.hasNext()) {
    ...
    if (file.getName().endsWith(".json"))
      options += '<option value="' + file.getId() + '">' + file.getName() + "</option>";
  }

  if (options.length > 0) {
    html +=
      "<div><b>Select drive backup:</b></div>" +
      '<div style="padding: 5px 0;"><select style="font-size:14px;padding:2px 5px;" id="restore">' +
      options +
      "</select></div>" +
...
function restore(file) {
  ...
  try {
    ...
    ui.alert("Unable to restore backup selected.", ui.ButtonSet.OK_CANCEL);
  }
  ...
  ui.alert("Restore complete. Please update Coins, Sparkline and Flux.", ui.ButtonSet.OK);
}
