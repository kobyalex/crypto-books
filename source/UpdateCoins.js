    ...
    for (i = 0; i < rows.length; i++) {
      if (rows[i] != undefined && rows[i].length > 0 && rows[i][0].length > 0) {
        ...
        if (market.hasOwnProperty(ticker)) {
          ...
          } else if (market[ticker].hasOwnProperty("current_price") && market[ticker]["current_price"] != null) {
          ...
          // Set fiat to Google Finance.
            ...
            sheet
              .getRange("N" + r)
              .setValue('=GOOGLEFINANCE("CURRENCY:' + getFiat().toUpperCase() + ticker.toUpperCase() + '")');
          }
